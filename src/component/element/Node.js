...
import { tuple2json, jsonExtend, jsonMap, jsonFilter } from 'src/common/json';
...
const Node = ({id, width=100, height=80, text, x, y, fill="#ffffff", mouseDown, schedule, selected}) => {
    ...
    return <g id={"node_"+id}  transform={"translate(" + x + "," + y + ")"} opacity="1.0">
        ...
        <path id={"node_schedule_"+id}
              d={"M0.0 5.0Q0.0 0.0 5.0 0.0L"+(width-5)+" 0.0Q"+width+" 0.0 "+width+" 5.0L"+width+" "+(height-5)+"Q"+width+" "+height+" "+(width-5)+" "+height+"L5.0 "+height+"Q0.0 "+height+" 0.0 "+(height-5)+"L0.0 5.0Z"}
              stroke="#858585" strokeWidth="2.0" strokeDasharray={352} strokeDashoffset={352*(1-(schedule||0))} opacity="1.0" fill={fill}/>
        ...
        <path id={"node_active_select_"+id}
              ...
              stroke="#858585" strokeWidth={dw} fill={"none"} opacity={selected?1:0} strokeDasharray={10}/>
...
const mountFollowMouse = (state, {id, e}) => {
    ...
    const shiftSelect = e.nativeEvent.shiftKey?
            Object.keys(select).indexOf(id)===-1?
                jsonExtend(newSelect,tuple2json(id,{ox:ox, oy:oy}))
                :
                jsonFilter(kv=>kv.key!=id)(newSelect)
            :
            Object.keys(select).indexOf(id)===-1?
                tuple2json(id,{ox:ox, oy:oy})
                :
                newSelect;
